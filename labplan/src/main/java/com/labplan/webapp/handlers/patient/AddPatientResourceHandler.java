...
public class AddPatientResourceHandler implements ResourceHandler {
	...
	public boolean doPost(HandlerParameters params) throws ServletException, IOException {
		...
		try {
			Patient parsedPatient = patientService.parse(request.getParameter("first_name"),
					request.getParameter("last_name"), request.getParameter("age"), request.getParameter("weight"),
